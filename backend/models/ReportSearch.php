...
class ReportSearch extends Model
{
    ...
    public function search($params)
    {
        ...
        // Загружаем параметры из формы поиска
        $this->load($params);
        ...
        if (!empty($this->period)) {
            $dates = explode(' - ', $this->period);
            if (count($dates) === 2) {
                $periodStart = date('Y-m-d', strtotime(trim($dates[0])));
                $periodEnd = date('Y-m-d', strtotime(trim($dates[1])));
                $query->andFilterWhere(['>=', 'date', $periodStart])
                      ->andFilterWhere(['<=', 'date', $periodEnd]);
            }
        }
