 ...
 */
class OrderProductBehavior extends Behavior
{
    /**
    ...
    public function saveRelations($event)
    {
        ...
        foreach ($this->_fields as $attributeName => $params) {
            ...
            foreach($oldValue as $oldId => $oldData) {
                if(isset($newValue[$oldId])) continue;

                // Return quantity of goods
                $prod = $this->getProduct($oldData['product_id']);
                if (!$prod) continue;
                $prod->q += $oldData['q'];
                $prod->save(false);
            ...
            foreach ($newValue as $newId => $newData) {
                $prod = $this->getProduct($newData['product_id']);
                if (!$prod) continue;
                ...
                // Add new records
                    ...
                    $model->product_id = $newData['product_id'];
